  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      if (searchData.length === 0) {
        ...
        btns.forEach((btn) => {
          if (parseInt(btn.dataset.qty) == 0) {
            btn.classList.add("no-qty");
        ...
        function setItem(item) {
          ...
          if (cartItems != null) {
            ...
            if (item.specs && cartItems[item.cartItemAdded].specs) {
              cartItems[item.cartItemAdded].specs = item.specs;
            }
      ...
      function getVar(sp, _i) {
                          ...
                          <div class="var_price">
                            <div class="_n_price">
                              UGX ${
                                i.getDiscount <= 0
                                  ? i.getPrice
                                  : i.getPrice * ((100 - i.getDiscount) / 100)
                              }
                            </div>
                            <div class="_d_price">${
        ...
        document.querySelector("#var_form").addEventListener("submit", (e) => {
          ...
          _varForm.forEach((v, k) => {
            if (k.charAt(0) !== "-") {
              if (k.charAt(0) == "_") {
                specs.Ingredients.push({
                  descriptions: k.slice(1),
                  qtty: v,
                  price: parseInt(v) * parseInt(_varForm.get(`-${k.slice(1)}`)),
                });
              } else if (k == "Sizes") {
                let arr = v.split("_");
                specs.Sizes[arr[0]] = arr[1];
              } else {
                specs[k] = v;
              }
            }
          });
          if (Object.keys(specs.Sizes).length == 0) {
            delete specs.Sizes;
          }
          if (specs.Ingredients.length == 0) {
            delete specs.Ingredients;
          }
          let cartItem = {
            cartItemAdded: _i,
            specs,
...
function _images(i, c) {
  let _s = new XMLHttpRequest();
  _s.open("GET", `/api/user/images/${i}`, true);
  _s.send();
  _s.onreadystatechange = () => {
    if (_s.readyState == 4 && _s.status == 200) {
      let _sr = JSON.parse(JSON.parse(_s.responseText)[0].image_path);
